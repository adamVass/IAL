...
***/
	...
	while(L->Act != NULL)
	{
		L->First = L->Act->ptr;		/* Nastaví nasledujúci prvok ako prvý */
...
**/
	...
	{
		tElemPtr temp = L->Act->ptr;		/* Do pomocnej premennej vlo¾í prvok zoznamu za akt. prvkom */
		L->Act->ptr = temp->ptr;			/* Ako nasledujúci prvok nastaví prvok za tým, kt. bude vymazaný */
		free(temp);
	}
}
...
**/
	if (L->Act != NULL)
	{
		...
		if ((insert = malloc(sizeof(struct tElem))) == NULL)			/* V prípade nedostatku pamäte vracia NULL, volá Error() a funkcia konèí */
		{
			Error();
			return;
		}
		insert->data = val;				/* Do dátovej èasti prvku ulo¾í hodnotu val */
		insert->ptr = L->Act->ptr;		/* Nový prvok bude ukazova» na prvok zoznamu za aktuálnym prvkom */
		L->Act->ptr = insert;			/* Aktuálny prvok bude ukazova» na novo pridaný prvok */
	}
}
...
**/
	if (L->Act != NULL)
		L->Act = L->Act->ptr;		/* Posunie aktivitu na ïal¹í prvok, ak ïal¹í neexistuje zoznam sa stáva neakt. */
}
